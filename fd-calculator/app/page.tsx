...
export default function FDCalculator() {
  ...
  // Use calculated values for results
  const { p: calculatedP, r: calculatedR, t: calculatedT } = calculated;

  // Simple Interest Formula
  const interest = (calculatedP * calculatedR * calculatedT) / 100;
  const maturity = calculatedP + interest;

  // Chart data
  const data = Array.from({ length: calculatedT }, (_, i) => ({
    year: i + 1,
    amount: Math.round(
      calculatedP + (calculatedP * calculatedR * (i + 1)) / 100
    ),
  }));
    ...
    <main className="h-screen bg-[#f5f0eb] flex items-center justify-center p-6">
      <div className="w-full max-w-5xl h-full max-h-[600px] bg-white rounded-2xl overflow-hidden shadow-md flex flex-col md:flex-row">
        ...
        <div className="flex-1 p-7 flex flex-col gap-4 overflow-auto">
          <div>
            <h1 className="text-2xl font-bold text-[#5c3d1e]">
              FD Calculator
            </h1>
          ...
          <div>
            <button
              ...
              className="bg-[#7a5c3c] hover:bg-[#5c3d1e] text-white rounded-full px-8 py-2.5 font-semibold text-sm transition-colors cursor-pointer"
            >
              Calculate
            </button>
          </div>
        ...
        <div className="flex-1 bg-[#d4bfaa] p-7 flex flex-col gap-4 overflow-hidden">
          ...
          <div className="flex gap-10">
            <div>
              <p className="text-xs text-[#7a5c3c] font-medium mb-1">
                Maturity Amount
              </p>
              <p className="text-2xl font-bold text-[#3a2510]">
                {fmt(maturity)}
              </p>
            </div>
          ...
          <div className="flex gap-4 shrink-0">
            ...
            <div className="flex-1 bg-[#5c3d1e] rounded-xl p-4 flex flex-col justify-between h-[110px]">
              <div className="flex items-start justify-between">
                ...
                <button className="w-6 h-6 rounded-full bg-white/20 flex items-center justify-center text-white text-xs shrink-0">
                  ↗
                </button>
              </div>
              ...
              <div>
                ...
                <p className="text-[#c9a98a] text-xs mt-0.5">
                  Exclusively For You
                </p>
              </div>
            </div>
            ...
            <div className="flex-1 bg-[#3a3028] rounded-xl p-4 flex flex-col justify-between h-[110px]">
              ...
              <div>
                ...
                <p className="text-[#c9a98a] text-[10px] mt-2 whitespace-nowrap">
